...
impl Hotkey {
    ...
    pub fn init(&self) -> Result<()> {
        ...
        let enable_global_hotkey = setup.latest().global_hotkey.unwrap_or(true);
        ...
        if let Some(hotkeys) = setup.latest().hotkeys.as_ref() {
            logging!(
                info,
                Type::Hotkey,
                true,
        ...
        let _ = manager.on_shortcut(hotkey, move |app_handle, hotkey, event| {
            if event.state == ShortcutState::Pressed {
                ...
                if hotkey.key == Code::KeyQ && is_quit {
                    ...
                    // 获取全局热键状态
                    let is_enable_global_hotkey = Config::setup_config()
                        .latest()
                        .global_hotkey
                        .unwrap_or(true);
